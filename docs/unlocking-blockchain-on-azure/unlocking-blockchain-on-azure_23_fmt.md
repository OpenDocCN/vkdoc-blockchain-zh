...
图 7-6 `Hyperledger Fabric` 中的多组织点对点网络

1.  `Hyperledger Fabric`：
    - 在 `Azure` 以太坊区块链网络中：
        - 链拓扑使用网络虚拟设备（`NVA`）进行部署。
        - `NVA` 在负载均衡器和对等节点服务器之后保持高可用性，从而提供一个容错的生态系统。
        - 链上使用 `NVA IP` 地址进行多节点链通信，而不是使用对等节点的实际 `IP` 地址，这使得侵入点对点环境变得更加困难。
    - 在 `Quorum`（可在 `Azure` 市场上获得）中：
        - 网络许可是一项功能，用于控制哪些节点可以连接到一个给定节点，以及该给定节点可以向外拨号连接到哪些节点。目前，它在启动节点时通过在单个节点级别使用 `--permissioned` 命令行标志进行管理。
    - 在以太坊中：
        - `RLPx` 传输协议：一种基于 `TCP` 的传输协议，用于以太坊节点之间的通信。
        - `Kademlia DHT`（分布式哈希表）在查找和加入网络时提供对等节点选择。该协议包含四个 `RPC`——`Ping`、`Store`、`Find Node` 和 `Find Value`。
2.  `Noise`：用于开发去中心化应用和加密协议的 `P2P` 网络栈，使用 `Go` 编写。用于 `Perlin Wavelet Network`。
    - 参考：[`https://github.com/perlin-network/noise`](https://github.com/perlin-network/noise)
3.  `Libp2p` 提供了建立对等网络的包。然而，由于开发者的语言偏好，它也有多种语言实现，例如：
    - `Go P2p`
    - 基于 `Kotlin` 的 `P2P`
    - `Js-Lib p2p`
    - `Rust Lib p2p`
...
- 其他框架，如 `Fabric`、`Sawtooth`、`Burrow`、`Iroha` 或 `Indy`，提供了与加密算法无关的选项。
- 以太坊框架：`Truffle`：
...
- 被定义为以太坊的开发环境、测试框架和资源管道，旨在让以太坊开发者的工作更轻松。
- 微软的 `Visual Studio Code` `IDE` 提供了 `Truffle` 服务以及 `Azure` 云服务，以增强开发和部署以太坊的 `DevOps` 体验。
- 该框架包含三个主要元素：
...
- 一个构建安全、高可用、高性能应用程序的框架，专注于多方计算和数据。
- 它本身不是区块链，而是通过分布式账本、多方计算和密码学来提供区块链的核心组件。
- 该框架提供了一个 `TEE`（可信执行环境），其中代码和数据以机密性和完整性被锁定在安全位置。
- 它为开发者提供了 `Open Enclave SDK`，使其能够以 `TEE` 形式集成，以实现链上逻辑和数据的真正去中心化和完整性，如图 7-8 所示。

![../images/474923_1_En_7_Chapter/474923_1_En_7_Fig8_HTML.jpg](img/474923_1_En_7_Fig8_HTML.jpg)

图 7-8：机密联盟框架

- 通常在开发任何区块链的共识时，硬分叉会被限制，并且除非所有成员同意，否则主机不得临时更改。因此，`Open Enclave SDK` 保证了这种开发的可靠性和稳定性。这使得区块链平台一旦部署在这样的框架上，就会对平台背后的主机/开发者保持中立。
- 该框架的六个主要元素如下（图 7-8）：
...
1.  服务总线并发
    ...
    - 服务总线包括队列机制、主题和订阅。
2.  哈希
    - `Azure` 借助 `Azure 逻辑应用`和 `Flow`，为这类动态函数提供了无服务器环境。
    - 在将数字资产上链时，可以使用数据和元数据的哈希版本来实现哈希。
3.  加密
    - `Azure 媒体服务`使用 `AES 128` 位明文加密密钥动态加密您的内容。
    - `Azure 存储`在将数据持久保存到云端时会自动加密您的数据。
    - `Azure 区块链服务`将这些编译到隔离虚拟网络中的安全加密组件内，使得随机黑客难以识别数据的准确来源。
    - 数据磁盘由 `Azure 磁盘加密`进行加密。
    - 共享访问签名为公共资产提供链上访问权限。
