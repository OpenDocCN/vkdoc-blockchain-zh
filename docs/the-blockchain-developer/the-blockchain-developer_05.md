...
注意索引从 0 开始，由于你有两笔交易，因此索引分别为 0 和 1。如果你有更多交易，索引会继续递增。图 4-10 展示了当你有两个 UTXO 时`listunspent`的结果。
...
你可能还记得，`vout`是交易中输出的索引号。在此示例中，我将指向一个`vout`并生成一个新交易。新交易可以包含多个其他`vout`，如图 4-11 所示。
...
要执行多签名者交易，您将创建两个独立的钱包用于测试。您可以在两台不同的机器上运行比特币核心，并使用 RPC 调用为每个钱包生成一个新的公共地址；或者您可以在 [`electrum.org/#download`](https://electrum.org/%2523download) 下载 Electrum 钱包，并在测试网模式下运行以生成您的第二个钱包。
