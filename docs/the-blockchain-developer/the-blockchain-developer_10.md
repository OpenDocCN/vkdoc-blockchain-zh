...
要导入你使用 Angular CLI `ng new` 命令创建的 `ethdapp` 项目，请打开 WebStorm，选择 **File** ➤ **Open**，然后导航到 `ethdapp` 目录。这样就完成了；WebStorm 会自动导入该项目。
...
```html
<mat-form-field>
  <input matInput placeholder="转账地址" formControlName="transferAddress" required>
  <mat-error *ngFor="let validation of account_validation_messages.transferAddress">
    <mat-error *ngIf="userForm.get('transferAddress').hasError(validation.type)">{{validation.message}}</mat-error>
  </mat-error>
</mat-form-field>

<mat-form-field>
  <input matInput placeholder="金额" formControlName="amount" required>
  <mat-error *ngFor="let validation of account_validation_messages.amount">
    <mat-error *ngIf="userForm.get('amount').hasError(validation.type)">{{validation.message}}</mat-error>
  </mat-error>
</mat-form-field>

<mat-form-field>
  <textarea matInput placeholder="备注" formControlName="remarks" required></textarea>
  <mat-error *ngFor="let validation of account_validation_messages.remarks">
    <mat-error *ngIf="userForm.get('remarks').hasError(validation.type)">{{validation.message}}</mat-error>
  </mat-error>
</mat-form-field>
```

最后，记得关闭 `div` 和表单，并包含一个提交按钮。

```html
<button mat-raised-button color="primary" type="submit">转账以太币</button>
```

</form>
```

对于 `transfer.component.css`，你将使用 `.transfer-container` div 来水平格式化你的表单。
...
就是这样。现在，你可以在浏览器中检查你的 dapp，你应该能够看到用户的默认数据，测试表单，验证它，并提交表单。参见图 9-10。

![../images/475651_1_En_9_Chapter/475651_1_En_9_Fig10_HTML.jpg](img/475651_1_En_9_Fig10_HTML.jpg)

**图 9-10** Ethdapp 转账页面，包含用户信息、验证器和提交按钮

你可以在此处下载此步骤：[`https://github.com/Apress/the-blockchain-developer/chapter9/step5.zip`](https://github.com/Apress/the-blockchain-developer/chapter9/step5.zip)。
