...
现在，通过传递 `RuntimeIdentifier` 设置参数，`publish` 指向运行时标识符 `osx.10.11-x64`。
...
图 7-7 — `docker_run_and_create_wallet` 脚本输出
...
图 7-8 — 正在运行的 `neo-privnet` Docker 容器
...
图 7-10 — 显示已领取代币的 `neo-privnet` 钱包
...
生成 `Neo.SmartContract.Framework` 项目
