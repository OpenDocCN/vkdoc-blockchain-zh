...
显示流通供应量中处于盈利状态的百分比的链上指标（数据来源：`https://studio.glassnode.com/metrics?a=BTC&m=supply.ProfitRelative&utm_source=gn_insights&utm_medium=insights_woc&utm_campaign=woc_25_2024`）
...
1.  **分析`获利中的供应量百分比`指标**
    访问 `Glassnode.com`（或同类网站）查看并使用`获利中的供应量百分比`这一链上指标。
...
比特币：活跃地址数量（数据来源：`https://studio.glassnode.com/metrics?a=BTC&category=&chartStyle=line&m=addresses.ActiveCount`）
