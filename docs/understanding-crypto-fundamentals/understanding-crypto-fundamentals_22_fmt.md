...
图 14-1：标准正态概率分布下最坏的 5%事件（即取值低于均值以下 1.645 个标准差的事件）
...
蒙特卡洛方法是一种统计方法，用于模拟某个过程中，在考虑众多变量时特定结果出现的概率。计算机对输入变量（例如，通货膨胀、利率、`GDP` 增长、加密资产采用率增长以及其他市场状况）运行数千次模拟。由于这些变量相互影响，分析其综合效应比独立分析它们要复杂得多。大量的模拟会以特定的可能性（例如，第 10^(个) 百分位）输出一个特定的投资组合价值。
