...
![](img/521550_1_En_3_Fig6_HTML.jpg)

一张显示 `truffle compile` 结果的截图，左侧菜单栏选中了 `truffle-config.js`。在屏幕主要部分有四个标签页：问题、输出、终端和调试控制台，其中终端标签页被选中。终端文本显示“正在编译你的合约。” 它已编译完成并等待输入。
...
![](img/521550_1_En_3_Fig7_HTML.jpg)

一张“导入账户”页面的截图。在“导入账户”标题下方，有一条信息显示“导入的账户将不会与你最初创建的 MetaMask 账户关联。” 下方，“选择类型”字段已选中“私钥”。再下方，用于输入私钥字符串的字段有一条虚线。底部的“导入”按钮被高亮显示。

**图 3-7** MetaMask：使用助记词导入现有钱包

点击网络列表，然后点击 `Localhost:8545`。使用 localhost 网络意味着你将把钱包指向你的本地开发区块链，如图 3-8 所示。

![](img/521550_1_En_3_Fig8_HTML.jpg)

一张“网络”页面的截图。顶部文字显示“显示或隐藏测试网络”，右侧有一个“关闭”按钮。列出的一些可见网络如下：Polygon Mumbai、Ropsten 测试网络、Kovan 测试网络、Rinkeby 测试网络等。Ropsten 测试网络左侧有一个勾选标记。底部的“添加网络”按钮。
...
![](img/521550_1_En_3_Fig9_HTML.jpg)

一张“添加代币”窗口的截图，有两个标签页：搜索和自定义代币。自定义代币标签页被选中。下方，“代币合约地址”栏显示地址，“代币符号”栏显示 `FIX`，“小数位数”栏显示 `0`。

**图 3-9** MetaMask：添加自定义代币

点击“添加代币”。代币符号以及你的余额将显示在此屏幕上（图 3-10）。

![](img/521550_1_En_3_Fig10_HTML.jpg)

一张“添加代币”窗口的截图，显示问题“是否要添加这些代币？”左侧是“代币”字样和一个标有 `FIX` 的彩色圆形图标。右侧是“余额”字样，显示 `1000 FIX`。
