...
提示：`Geth` JavaScript 控制台提供了一个交互式控制台，让你可以使用 JavaScript 语言与以太坊区块链进行交互。
...
提示：`web3` 对象（来自 `web3.js` 库）允许你以编程方式与以太坊区块链交互。它还暴露了 `eth` 对象，而 `eth` 对象本身也暴露了 `personal` 对象。因此，`personal.newAccount()` 函数的完整名称实际上是 `web3.eth.personal.newAccount()`。`personal` 对象允许你与以太坊节点的账户进行交互。
...
提示：如前所述，`eth` 对象源自 `web3` 对象，因此上述函数等同于 `web3.eth.getBalance(web3.eth.accounts[0])`。
