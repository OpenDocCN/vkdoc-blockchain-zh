# Stake Hodler 资本主义：区块链和 DeFi（去中心化金融）
...
如果一个用户干扰了比特币的交易记录，所有剩余的节点将很容易交叉引用并指定具有错误信息的节点。该系统有助于建立准确透明的事件顺序。对于比特币来说，这些信息就像是交易清单；但是，对于一个区块链来说，可能需要存储不同的法律合约、州身份或公司的产品库存。几个分散网络的计算能力将需要在规定的变化上一致，以使系统如何运行或容纳的信息有所不同。这确保了发生的任何变化都符合大多数人(如果不是所有人)的最佳利益。
...
美联储控制着美元。从这个中央权威体系来看，一个用户的数据和货币基本上是由他们的银行或政府支配的。如果用户的银行被黑，那么客户的个人信息就有危险。如果客户的银行停止运作，或者他们生活在一个政府不稳定的国家，他们的货币价值可能会有风险。2008 年，一些资金短缺的银行用纳税人的钱获得了救助。这些担忧孕育了著名的比特币的诞生。
...
比特币是区块链潜在低效率的最好例子。比特币的 `PoW`（工作证明）系统包含一个新的区块链区块大约需要 10 分钟。在这种速度下，据计算，区块链网络每秒只能处理大约 7 个事务（`TPS`）。尽管以太坊等其他加密货币的功能优于比特币，但它们仍受到区块链的限制。举例来说，传统的签证系统最多可以处理 24，000 份 `TPS`。
...
由法定货币支持的加密货币是最常见的，也是市场上发行的第一种固定货币。
...
价格的下降是由于自动做市商的性质(`AMM`)。交易越大，对价格的影响越大。例如，如果您使用一致的 `AMM` 产品，所有交易必须保持产品 `XY = k` 不变，其中 `x` 和 `y` 是池中两种加密货币（或令牌）的数量。输入大小`δx` 越大，最终的 `y/x` 比越低，给出交换价格。这个问题通常对大公司或小流动性池很重要。
...
`DeFi` 仍然是一个即将到来的领域，伴随着智能合约错误和漏洞的风险。一些创新的保险替代品已经进入市场，以帮助用户购买保险来保护他们的财产。`Nexus Mutual` 就是提供的解决方案之一。它给出了一个智能合约封面，防止无意中使用智能合约代码。

# DeFi 生态系统：服务、用例与未来

## 核心服务与基础设施

DeFi 生态系统的另一个非常重要但并非非常有限的财务部分是 Oracle 的服务，专注于在智能合约之外提供稳定的数据流。这个领域最受欢迎的项目是 `Chainlink`。这些几乎存在于 DeFi 生态系统的每一个主要部分。也可以几种方式组合。

你可以把它看作是“金钱乐高”，因为你可以在现有的积木上建造更复杂的 DeFi 产品。
...
此外，占主导地位的资产管理基金开始认真对待 `DeFi`。这一类别中最著名的是 `Grayscale`，它是世界上最大的加密投资账户。它管理着超过 `52` 亿美元的加密投资，包括 `44` 亿美元的比特币。
...
即将推出的以太坊 2.0 和 Tier 2 扩展解决方案可以帮助解决这个问题。它还具有适用于其中一个协议的微妙功能或变化，使用户采取不太明显的行动，可以跨多个协议级联。一个很好的例子是最近`复合协议`成分的增加，这迫使消费者参与看似无利可图的高利率贷款，由于额外的补偿而有利可图。这些情况可能非常危险，但正如你已经注意到的，它们使整个生态系统更加强大，对未来类似情况不那么敏感。
...
高产农业技术正在发展和试验中，但其意义是惊人的。在普通互联网上，除非你向网站所有者提供足够的数据来激活交易，否则你无法购买产品，但通过 `DeFi`，你甚至可以在不透露姓名或任何个人信息的情况下借钱。`DeFi` 应用程序并不关心，因为它有自己提供的抵押品来确保债务(例如，在`复合平台`上，10 美元的贷款需要价值约 20 美元的抵押品)。
...
顺便说一下，这就是为什么 `Uniswap` 价格通常是正确的，因为交易者在较大的市场中发现一些偏差，并非常快地进行交易以获得仲裁收益。然而，如果 500，000 `USDC` 和 500，000 戴在池中，1 戴兑换 1 `USDC` 交易对相对价格几乎没有影响。这就是流动性如此有用的原因。可以把资产锁定在一个`复合平台`，小赚一笔。高产养殖似乎吸引了更广泛的散户群体，而寻找利润最大化方法的专业投资者是那些从事高产养殖的人。
...
成功的高产农民利用非常复杂的系统或策略。他们定期在不同的借贷市场之间转移加密货币，以实现利润最大化。他们也会对自己使用的最佳耕作策略守口如瓶。为什么？他们越是暴露自己的策略，效果就越差。

产量农业是分散融资 (DeFi) 的森林地块，农民参与竞争以种植最好的作物。DeFi (分散金融) 运动一直处于区块链创新的最前沿。是什么让 dApps 如此特别？它们没有被授权，所以任何有网络连接和二级钱包的人都可以和它们互动。此外，dApps 消除了对托管人或经纪人的需求。

我可以强迫它工作；为什么我可以使用该资产？那么，高产农民是如何获得产量的呢？下面我会一一解释。

## 高产农业:概述

有利可图的收益农业也称为流动性开采，利用加密货币赚取利息。简而言之，这意味着质押加密货币，并收到利息的质押金额。

在某种意义上，这个过程在本质上非常类似于货币的质押，但它的工作方式不同，有许多复杂的背景。通常，用户被称为流动性提供者 (LP)，他们向流动性池中添加现金，发挥着巨大的作用。

流动性池只是一个存放资金的智能合约。流动性提供者因向资金池提供流动性而获得报酬。奖励可以通过基础 DeFi 平台或其他来源产生的佣金获得。
...
然而，跨链链接和其他类似的改进可能会使 DeFi 应用程序在未来与底层区块链平台分离。这意味着任何智能合约都将与支持合约功能的任何区块链兼容。

为了寻求高收益，农民们通常会在不同的协议之间转移他们的资金。因此，DeFi 平台还可以提供其他财务激励措施，以吸引更多资本进入该平台。像中央证券交易所一样，流动性吸引更多的流动性。

是什么引发了热潮？对高产农业的突然兴趣可能与 `COMP` 令牌的推出有关，这是一种复杂的金融生态系统管理令牌。控制令牌赋予持有者治理权。但是，如何分配这些令牌以使网络尽可能分散呢？启动分散式区块链的一种常见方式是通过算法分发这些控制令牌，作为流动性激励。这意味着流动性提供者将培育新令牌，并为协议提供流动性。

虽然该公司没有发明产量农业，但 `COMP` 的发布使这种类型的加密分发模型更受欢迎。从那时起，其他 DeFi 项目提供了创新的计划，为生态系统带来流动性。

## TVL（总价值锁定）

衡量 DeFi 农场整体健康状况的最佳方法是监控 `TVL`（锁定总值）。它衡量有多少加密货币被锁定在 DeFi 借贷平台和其他类型的货币市场。

从某种意义上说，`TVL` 是流动性池中的总流动性。这是一个有用的指标来衡量 DeFi 的健康和整个农业市场。这也是比较不同 DeFi 协议的“市场份额”的有效方法。

追踪 `TVL` 的好地方是 DeFi Pulse。你可以看到哪些平台在 DeFi 上拥有最多的 `ETH` 或其他加密资产。这可以提供当前产量农业情况的概况。自然，更高的质押价值导致更高产量的农业经营。你可以用瑞士法郎、美元甚至 `BTC` 来估算 `TVL` 的价值。每个人对 DeFi 的货币市场状况都有不同的看法。
...
存放的资金通常是由美元支持的稳定的货币；但是，这可能会有所不同。在分散金融中常用的稳定币有 `DAI`、`USDC`、`USDT` 等。一些协议是代币，代表存入智能合约或系统的货币。比如把 `DAI` 加到 `Compound`，就会得到 `cDAI` 或者 `Compound DAI`。把 `ETH` 插入 `Compound` 中，你就会得到 `cEthereum`。
...
因此，让我们假设你需要一个 200%的安全率的贷款协议。这意味着你投入的每 100 美元可以借 50 美元。然而，增加比必要数量更多的抵押品以进一步降低清算风险通常更安全。也就是说，许多系统使用非常高的质押比率（例如，750%）来使整个平台相对安全，避免清算风险。

## 固有风险

作为一名高产农民取得成功并不是一件容易的事情。最赚钱的增长策略非常复杂，只推荐给高级用户。此外，产量农业往往更适合于那些能获得大量被称为“鲸鱼”的秘密资源的人

你必须知道你在做什么，这样才不会赔钱。除了清算你的投资基金，让我们看看另一个与收益相关的风险——农业。高产农业的一个显而易见的危险是一份精明的合约。由于 DeFi 的性质，小团队在有限的预算下创建和开发许多协议。这可能会增加感知合约错误的风险。

即使大型协议由声誉良好的审计公司审计，弱点和错误仍会不断被发现。区块链的防篡改和不可改变的特性会导致资金在有缺陷的智能合约中被无限期锁定。但是，随着每天为智能合约创建越来越多的平台，我们可以很容易地看到新的创业公司创建一个抽象层，在合约发布到区块链之前，给你一个测试期。在智能合约中下注资金时，应考虑到这一点。DeFi 最大的好处之一也是它最大的问题之一。这就是可组合性的问题。让我们看看这是如何发生的。
...
`COMP` 是一种算法货币交易所，消费者可以在这里借入和质押资产。任何拥有以太坊钱包的人都可以将资产质押入 `Compound` 流动性池，并立即获得质押金额的回报。价格根据供求关系通过算法进行调整。`COMP` 是产量农业技术中最重要的协议之一。

### 复合高产农业

使用的平台：`Maker`、`Compound`、`Curve` 和 `InstaDapp`。

*   通过使用稳定币优化您的计划来增加您的红利资产。

# DeFi 协议与收益策略

## Maker

Maker 是一个去中心化的借贷平台，支持创建“DAI”，一种通过算法与美元价值挂钩的稳定货币。用户可以打开`Vault`来锁定`ETH`、`BAT`、`USDC`或`WBTC`等安全资产。他们可以用抵押证券产生贷款。这一借来的数额随着时间的推移会产生利息，称为稳定费用，由`MKR`代币持有人决定。高产农民可以利用`DAI`造币厂的生产策略。

## Synthetix

Synthetix 是一个合成资产协议。任何人都可以安全地存放或持有`Synthetix Network Token (SNX)`或`ETH`，并为其发行 Synthetix 资产。什么是合成资产？几乎所有东西都有稳定的价格流。这允许您将几乎任何金融资产添加到 Synthetix 平台。Synthetix 可以将所有类型的资产用于即将到来的高产农业。如果你计划利用长期黄金袋作为你的收益农业策略，合成资产可能是合适的选择。
...
`Curve Finance`是一个分散的交换协议，专门为固定货币的有效交换而设计。与其他类似`Uniswap`的协议不同，`Curve`允许用户以相对较低的滑动率交换昂贵的固定货币。正如你可能已经猜到的，`Curve`池是框架的重要组成部分，因为它富含稳定的货币用于产量农业。
...
该平台利用`Ren`协议、`Curve`、`Synthetix`、`Balancer`赚取`SNX`、`Ren`、`BAL`和`CRV`，以确保 Curvepool `sBTC`的流动性。

- 使用`Ren Bridge`将比特币转移到以太坊。
- 将新获得的`renBitcoin`输入 Curvepool `sBTC`。
- 在 LP 奖励区通过`Mintr`提交`SBTC`机票。
- 以/任平衡代币的形式获取价格。
- 在你的资产负债表上获得和/或任的报酬。

曲线平台上的`CRV`流动性挖掘利用`Curve`、`Synthetix`、`Ren`协议和`yearn.finance`。用`Curve`和`Curve DAO`流动性指标系列增加你的`CRV`收益。

- 将抵押品支付给七个曲线流动性池之一。
- 那些希望参与比特币池的人使用`Ren`协议将`BTC`转移到以太坊。
- 通过曲线评估投资流动性代币。
- 为了增加流动性，我们使用`Curve`锁定`CRV`。
- 为`sUSD`和`sBTC`池寻找额外的激励措施，如`SNX`和`Ren`。

`Curve`是一个自动化做市商平台，在许多方面与`Uniswap`和`Balancer`类似，只是它只支持包含具有类似行为的资产的流动性池，如固定货币，或那些包装的类似资产，如`wBTC`和`tBTC`。这种方法允许`Curve`使用更有效的算法，并在任何分散的以太坊交易所提供最低的费用、滑动和波动损失。

## Balancer

`Balancer`是`Uniswap`和`Curve`流动性协议。但是，主要区别在于，流动性资金可以调整代币的分配。这允许流动性提供者创建他们的平衡池，而不是`Uniswap`所要求的 50/50 分割。像`Uniswap`一样，有限合伙人收取流动资金交易的费用。

`Balancer`具有建立流动性的灵活性，是作物种植策略的重要创新者。

### Balancer 流动性采矿

该平台将您的资金连接到`Balancer`的流动性池，以赚取余额控制令牌。使用类似预测交换的预测表，根据总百分比来确定哪个池接收的`BAL`最多，其中 1 是可能的最高比率。
...
例如，辛西娅用比特币从在线零售商那里购买了一台电视。为了确保她的比特币是真实的，矿工们执行一种特定于每个区块链的计算操作，以验证交易并添加新的交易块。验证过程要复杂得多，但是为了简单起见，我们用最简单的话来说，就是对事务的验证。所有的交易都用一个叫做“区块链”的虚拟挂锁以区块形式进行汇编。矿工然后运行软件，找到适合打开这个挂锁的钥匙，以添加新的块。

当他们的计算机找到这个密钥时，盒子就会打开，交易就会得到验证。找到打开挂锁的合适钥匙的奖励相当于 12.5 个新生成的比特币，由于其假名发明者`Satoshi Nakamoto`为比特币创建的预定义规则，该价值将每一定数量的区块减半。根据一个最新的真实比特币交易的顶级网站，目前找到密钥所需的尝试次数约为 1789546951.05 次。

不考虑巨大的尝试次数，大约每 10 分钟就提供 25 个比特币的奖励。然而，如今独自采矿是令人沮丧的，因为寻找石块的过程是如此受欢迎，寻找石块的难度更小，以至于你可能需要将你的机器搁置大约三年才能产生任何货币。
...
一旦你创建了你喜欢的钱包，你就会得到一个类似于`1BEkUGADFbrEShQb9XJ`的地址。这是一种直接将你的比特币发送到你的钱包的方式，所以请注意。你可以在`Coinbase`的关联账户下找到钱包地址。
...
不同的论坛有不同的挖掘选项，`OSX`用户可能会发现自己有点麻烦。矿工利用空闲的 GPU 周期为采矿作业提供动力。矿工利用这些周期来帮助处理与比特币相关的用户对用户流程。因此，通过做“工作”，你也在处理网络。你可以对此进行更多的研究，以获得更清晰的图片。给你一个起点，查找`ASICs`（专用集成电路）。
...
然而，在智能合约程序的帮助下，交易是在人与人之间直接完成的，而不是通过加密货币交易所这样的中央中介进行交易。`DApps`通过`Web3`浏览器扩展或应用程序（如`MetaMask`）访问，允许用户通过网站与以太坊区块链互动。

此外，这些`DApps`中的大多数可以连接并协调工作，以创建详细的金融服务。例如，稳定货币持有者可以将资产转化为流动性池。其他人将通过提供额外的抵押品从这个资金池中借款，通常比贷款金额更多。然后，保单必然会根据对资产的不同时间要求改变利率。

分散化意味着没有中央交易所。用于`DeFi`政策本身的智能合约程序是在开源软件的帮助下运行的，这些开源软件是由开发人员和程序员社区提供的。`DeFi`政策的一个例子是`Uniswap`。这是一个在以太坊区块链上运行的分散式交易所或`DEX`，允许在以太坊区块链上生产的数百种不同的数字代币的交易。

`Uniswap`的算法不是依靠集中的市场标记来完成订单，而是鼓励用户通过向提供流动性的人收取交易费来为代币创建流动性池。虽然用户完全控制`Uniswap`平台，但开发团队负责为其部署编写软件。

直接购买并不是大公司控制的唯一一种交易或合约，其他金融应用如贷款、保险、衍生品、博彩、众筹等也在他们的掌控之中。因此，消除所有这些交易中的中间人是`DeFi`的主要目标之一。

我们已经介绍了最新的加密货币，如加密农业和`DeFi`产品。这场先进的经济革命还能带来什么？我们不知道基于这些当前的组件，未来会有什么新的应用。然而，不可靠的流动性协议和其他`DeFi`产品处于经济学、密码经济学和计算机科学的前沿。

`DeFi`的货币市场无疑有助于创建一个更加开放和可访问的金融系统，任何有互联网连接的人都可以访问，这将弥合我们在地球上的生活之间的差距，并依赖于人的因素来潜在地实现在火星上的生活。与此同时，我们从基于地球证券的产量农业中获得被动收入。在“`Stake Hodler Capitalism`”系列图书的未来部分，我们打算深入探讨区块链的许多应用，包括智能合约、物联网、零售、农业和制造业。