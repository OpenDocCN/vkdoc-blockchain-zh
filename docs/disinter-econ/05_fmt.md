...
David Shrier^([1](#Aff3) [ ](#ContactOfAuthor1))(1)Imperial College London, London, UKDavid ShrierEmail: `david@visionaryfuture.co`
...
我们尚未达到能够充分利用分布式数据潜力的技术以及法律和商业框架的发展阶段。此外，如果我们希望个人能够欣赏、完全理解并利用分布式数据带来的益处，就需要提高数据素养。在这些行动能够发生之前，我们必须解决核心基础设施挑战，为分布式数据的未来创造必要条件：
...
数据系统通过应用人工智能变得几何级更有用，而先进的 `AI` 系统，如机器学习和深度学习，由大量数据驱动。因此，数据讨论和 `AI` 讨论迅速趋同。牛津互联网研究所的卢西亚诺·弗洛里迪(`Luciano Floridi`)等技术学者，基于对众多伦理框架和模型的审查，提出了一个 `AI` 伦理的框架方法，归纳为伦理 `AI` 的 5 大支柱^([58](#Fn58))，总结如下：
...
通过将代码带入数据，而不是将数据带入代码，我们有机会显著提高信息安全。当代信息管理系统，尤其是区块链或当前一代的分布式账本技术，为管理这些更复杂的信息流提供了健壮的代码架构。实施近同态加密的下一代系统，如 `Enigma`，提供了在信息安全和加密方面实现指数级改进的潜力，同时为数据系统保持足够的灵活性和可访问性，使其在多个应用中发挥作用（图 [5.1](#Fig1)）。^([61](#Fn61))![img/492933_1_En_5_Chapter/492933_1_En_5_Fig1_HTML.png](img/492933_1_En_5_Chapter/492933_1_En_5_Fig1_HTML.png)
...
提供更多数据素养的模式已经存在。已经提出了由数据驱动的大规模社区参与框架，提出了如何将数据用于公共利益的问题，并声称分布式数据中内置的控制机制(如 `OPAL`)可以管理个人数据隐私和社会利益之间的紧张关系。 ^([66](#Fn66)) 通过一系列协调一致的行动，数据素养可以被引入社会的不同领域，并为促进分布式数据经济创造必要的要素。
...
**适应与创新支持**。`GDPR` 和 `PSD2` 值得进行积极审查和加强，隐私和数据可移植性法规更应如此（特别是考虑到其他司法管辖区如何将 `GDPR` 和 `PSD2` 用作模型）。既然欧洲已经有时间观察企业如何尝试在实践中遵守，就可以对框架和解释指南进行调整。例如，`GDPR` 的一个意外后果是使小型公司更难遵守，并为初创企业带来了新的成本、业务和财务风险，尽管也可能创造了机会领域。^([69](#Fn69)) 监管机构和政策制定者可以采取的调节这些法规对创新影响的步骤包括：
...
2.  (2)
    ...
    为初创企业提供更多的“*沙盒*”机会，在一个封闭的环境中与监管机构接触，在这里可以坦率地提出和解决问题，初创企业内部的合规能力也得到提高。在监管者和决策者的参与下，围绕分布式数据、数据隐私和数据可移植性的“技术冲刺”和黑客马拉松成为另一种机制，可以同时围绕新技术建设政府能力，并将私人部门活动与机会领域结合起来。
...
通过协调这些领域的稳健治理、适应与创新支持以及协调，可以更好地管理分布式数据所代表的中断，实现经济效益，同时减轻对社会的潜在危害。
