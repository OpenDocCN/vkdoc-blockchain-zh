...
-   用于访问 `value` 的 getter 函数，由字段声明中使用 `public` 关键字生成
...
代码清单 2-7
用于创建 Counter web3 合约对象的函数。请注意，该函数不会部署新合约，它只是创建一个 JavaScript 对象，作为此前在指定地址部署的合约的包装器。
```
...
代码清单 2-8
用于获取部署在 Rinkeby 网络上的 Counter 合约的 web3 合约实例的代码。请注意，为了避免硬编码地址，你也可以将地址存储为环境变量，并通过 process.env37 进行检索。
```
...
代码清单 2-9
用于渲染 Counter 合约并与之交互的空 React 组件。我们将迭代地为其添加功能
```
...
代码清单 2-11
组件挂载时检索 Counter 的初始值
```

请注意调用 counter 合约实例以检索初始值的方法。web3js 的调用 API 可能看起来有些别扭，但它背后有其逻辑：
...
代码清单 2-12
用于显示计数器值的渲染方法。请注意，该值仅在初始查询合约返回后才可用，因此我们需要处理值尚未就绪的情况
```
...
Metamask 确认对话框，用于接受应用程序发出的交易。每次您尝试代表用户发送交易时，用户都会看到此对话框。
...
代码清单 2-16
更新后的 render 方法，用于在交易待处理或失败时显示通知，并在交易完成前禁用按钮
```
...
代码清单 2-15
更新后的 increaseCounter 函数，用于跟踪一个标识符以识别待处理的交易和任何返回的错误
```
...
代码清单 2-17
订阅 Increased 事件，每当发出事件的新实例时，该订阅会更新组件的状态
```
